...
//
// The former `stage-N.inject` signal-file mechanism was removed in favour of
// the manager-driven feedback gate (see tests/feedback-gate.test.js). The
// helper below still exercises injectLoopStages directly to keep the splice
// / re-index / loop-counter contract regression-tested.
// ---------------------------------------------------------------------------

describe('injectLoopStages — splice, re-index, and loop-counter', () => {
  test('inserts stages at run.currentStage, re-indexes, bumps counter', () => {
    delete require.cache[require.resolve('../src/services/pipelineManager')];
    const pm  = require('../src/services/pipelineManager');
    const run = {
      currentStage: 2,
      ...
      stageStatuses: [
        ...
        { index: 1, agentId: 'b', status: 'completed' },
        { index: 2, agentId: 'c', status: 'pending'   },
      ],
      loopCounts: {},
    };
    pm.injectLoopStages(run, ['a', 'b'], 'b');
    assert.deepEqual(run.stages, ['a', 'b', 'a', 'b', 'c']);
    assert.equal(run.loopCounts['b'], 1);
    run.stageStatuses.forEach((s, i) => assert.equal(s.index, i, `stage ${i} re-indexed`));
    assert.equal(run.stageStatuses[2].status, 'pending', 'injected stage starts pending');
  });
});
...
describe('stall watchdog — harness awareness', () => {
  ...
  test('pi declares that it does not stream progress', () => {
    assert.equal(getAdapter('pi').streamsProgress, false,
      'pi buffers until exit; the watchdog must not judge it by log growth');
  });

  test('the streaming harnesses are still watched', () => {
    for (const id of ['claude', 'opencode']) {
      assert.notEqual(getAdapter(id).streamsProgress, false,
        `${id} streams as it works, so its log going quiet is real evidence`);
    }
  });
});
