...
describe('Pipeline prompt endpoints — REST integration', () => {
  ...
  // ── POST /api/v1/agent/prompt — Inksmith fields (T-006) ───────────────────

  describe('POST /api/v1/agent/prompt — Inksmith source + refinementId fields', () => {
    test('response includes source field when inksmith is disabled (default)', async () => {
      const agentId = 'senior-architect';
      const { spaceId: sId, taskId: tId } = setupSpaceViaManager(dataDir);
      ...
      assert.strictEqual(res.status, 201, `Expected 201, got ${res.status}: ${JSON.stringify(res.body)}`);
      assert.ok('source' in res.body, 'response must include source field');
      assert.strictEqual(res.body.source, 'local-fallback',
        'source must be "local-fallback" when inksmith is disabled');
    });

    test('response includes refinementId field (null when inksmith is disabled)', async () => {
      const agentId = 'senior-architect';
      const { spaceId: sId, taskId: tId } = setupSpaceViaManager(dataDir);
      ...
      assert.strictEqual(res.status, 201);
      assert.ok('refinementId' in res.body, 'response must include refinementId field');
      assert.strictEqual(res.body.refinementId, null,
        'refinementId must be null when inksmith is disabled');
    });

    test('written file content matches the promptFull returned in response', async () => {
      const agentId = 'senior-architect';
      const { spaceId: sId, taskId: tId } = setupSpaceViaManager(dataDir);
      ...
      assert.ok(typeof promptPath === 'string' && promptPath.length > 0, 'promptPath must be a non-empty string');

      const fileContent = fs.readFileSync(promptPath, 'utf8');
      assert.strictEqual(fileContent, promptFull,
        'written file content must match promptFull in the response');
    });
  });
});
