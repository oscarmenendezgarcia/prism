...
function buildKanbanBlock(spaceId, taskId, isLastStage = true) {
  ...
  return [
    ...
    '  mcp__prism__kanban_add_comment({ <the Space ID and Task ID above>, author: "<agent-id>", type: "question", text: "<question + both options>", targetAgent: "<agent-id or omit>" })',
    ...
    '    mcp__prism__kanban_add_comment({ <the Space ID and Task ID above>, author: "<your-agent-id>", type: "note", text: "Assumption: <what + why>" })',
    '  • Blueprint deviation: you decided to do something differently than specified',
    '    mcp__prism__kanban_add_comment({ <the Space ID and Task ID above>, author: "<your-agent-id>", type: "note", text: "Deviation: <what changed + why>" })',
    '  • Non-trivial trade-off: you chose approach A over B and the reason is not obvious',
    '    mcp__prism__kanban_add_comment({ <the Space ID and Task ID above>, author: "<your-agent-id>", type: "note", text: "Trade-off: chose <A> over <B> because <reason>" })',
    '  • Hard-won lesson: you hit a non-obvious failure and solved it (these notes feed the Folio knowledge base)',
    '    mcp__prism__kanban_add_comment({ <the Space ID and Task ID above>, author: "<your-agent-id>", type: "note", text: "Lesson: <what failed> — root cause: <cause>. Fix: <fix>" })',
    '',
    'HANDOFF SUMMARY — post BEFORE you finish your stage. EVERY stage posts one, not just the last: the next stage reads it to know what exists and what was decided. An intermediate stage does not move the task to done, but still posts its handoff.',
    '  mcp__prism__kanban_add_comment({ <the Space ID and Task ID above>, author: "<your-agent-id>", type: "note", text: "Handoff: produced <artifacts>. Next agent should read <key files>. Folio pages used: <slugs, or none>." })',
  ].join('\n');
}
